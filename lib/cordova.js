RemoteAutoupdate = function(url) {

  var connection = DDP.connect(url);
  AutoupdateCordova(connection, url);

}

var AutoupdateCordova = function(connection, remoteUrl) {

  if (!connection || !remoteUrl) return;
  if (remoteUrl[remoteUrl.length-1] !== "/") remoteUrl = remoteUrl + "/";
  ...
  ClientVersions = new Mongo.Collection("meteor_autoupdate_clientVersions", { connection: connection });
  ...
  var onNewVersion = function () {
    ...
    var urlPrefix = remoteUrl + '__cordova';
